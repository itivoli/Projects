...
always @(posedge clk or posedge reset) begin
    // Reset logic.
    if(reset) frame_timer_reg <= 0;

    // Update animation frame timer.
    else frame_timer_reg <= frame_timer_next;
end
...
always @ (frame_timer_reg or cd) begin
    if(frame_timer_next < FRAME_CNT_1) begin
        if(cd == CD_U) rom_offset_next <= U_1;
        else if(cd == CD_D) rom_offset_next <= D_1;
        else if(cd == CD_L) rom_offset_next <= R_1;
        else if(cd == CD_R) rom_offset_next <= R_1;
    end
    else if(frame_timer_next < FRAME_CNT_2) begin
        if(cd == CD_U) rom_offset_next <= U_2;
        else if(cd == CD_D) rom_offset_next <= D_2;
        else if(cd == CD_L) rom_offset_next <= R_2;
        else if(cd == CD_R) rom_offset_next <= R_2;
    end
    else if(frame_timer_next < FRAME_CNT_3) begin
        if(cd == CD_U) rom_offset_next <= U_1;
        else if(cd == CD_D) rom_offset_next <= D_1;
        else if(cd == CD_L) rom_offset_next <= R_1;
        else if(cd == CD_R) rom_offset_next <= R_1;
    end
    else if(frame_timer_next < FRAME_CNT_4) begin
        if(cd == CD_U) rom_offset_next <= U_3;
        else if(cd == CD_D) rom_offset_next <= D_3;
        else if(cd == CD_L) rom_offset_next <= R_3;
        else if(cd == CD_R) rom_offset_next <= R_3;
    end
end

always @(posedge clk or posedge reset) begin
    // Reset logic.
    if(reset) rom_offset_reg <= U_1;
     
    // Update rom offset.
    else rom_offset_reg <= rom_offset_next;
end
