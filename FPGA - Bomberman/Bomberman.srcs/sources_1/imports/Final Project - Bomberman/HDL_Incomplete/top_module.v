...
(
   input wire clk, rst, pause,                              // clk from oscillator, rst from asynchronous button
...
debounce_button db_u(.clk(clk), .reset(reset), .button(PB_UP)   , .pause(pause), .db_out(U));
debounce_button db_d(.clk(clk), .reset(reset), .button(PB_DOWN) , .pause(pause), .db_out(D));
debounce_button db_l(.clk(clk), .reset(reset), .button(PB_LEFT) , .pause(pause), .db_out(L));
debounce_button db_r(.clk(clk), .reset(reset), .button(PB_RIGHT), .pause(pause), .db_out(R));
debounce_button db_a(.clk(clk), .reset(reset), .button(PB_ENTER), .pause(pause), .db_out(A));
...
block_module block_module_unit(.led(led), .clk(clk), .reset(reset), .display_on(display_on),
...
bomb_module bomb_module_unit(.clk(clk), .reset(reset), .pause(pause), .x_a(x_a), .y_a(y_a), .cd(current_dir_reg), 
...
assign rgb = (display_on) ? rgb_reg : 12'h00;
